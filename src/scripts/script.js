...
function fetchCoordinatesAndData() {
    ...
    const geocodeApiUrl = `https://geocode.maps.co/search?q=${encodeURIComponent(address)}`;
...
function fetchData(latitude, longitude, locationName) {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            const { sunrise, sunset, first_light, last_light, dawn, dusk, solar_noon, golden_hour, day_length, timezone } = data.results;
